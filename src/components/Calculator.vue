    ...
    digitHandler(num) {
      if (num !== '') {
        ...
        if (this.total !== '' && this.total.length >= 2) {
          ...
          if (
              this.total.charAt(this.total.length - 2) === '/'
              && this.total.slice(-1) === '-'
              && this.operations.includes(String(num))) {
            return;
          }


          if (this.operations.includes(this.total.charAt(this.total.length - 2)) && this.total.slice(-1) === '0' && String(num) !== '-') {
            this.total = this.total.slice(0, -1)
          }

          if (this.operations.includes(String(num)) && this.operations.includes(String(this.total.slice(-1)))) {

            if (this.total.slice(-1) === '*' && num === '-') {
              this.total = JSON.parse(JSON.stringify(this.total));
            } else if (this.total.slice(-1) === '/' && num === '-') {
              this.total = JSON.parse(JSON.stringify(this.total));
            } else if (this.total.slice(-1) === '/' && num === '/') {
              this.total = this.total.slice(0, -1)
            } else {
